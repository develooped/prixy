 ...
 *
 * Options:
 *   - `apiVersion`    (optional) the Dropbox API version to use (either '1' or '2'). Default is '2'.
...
function Strategy (options, verify) {
  ...
  var defaultOptionsByApiVersion = {
    1: {
  ...
  if (
    ...
    supportedApiVersions.indexOf(options.apiVersion.toString()) === -1
  ) {
    throw new Error(
      'Unsupported Dropbox API version. Supported versions are "1" and "2".'
    )
  }

  this._apiVersion = options.apiVersion || '2'

  options.authorizationURL =
    options.authorizationURL ||
    defaultOptionsByApiVersion[this._apiVersion].authorizationURL
  options.tokenURL =
    options.tokenURL || defaultOptionsByApiVersion[this._apiVersion].tokenURL

  options.scopeSeparator =
    options.scopeSeparator ||
    defaultOptionsByApiVersion[this._apiVersion].scopeSeparator
  options.customHeaders =
    options.customHeaders ||
    defaultOptionsByApiVersion[this._apiVersion].customHeaders

  OAuth2Strategy.call(this, options, verify)
  this.name = 'dropbox-token'
}
